...
export class Metadata {
    private _metadata: Map<string, Object>;
    ...
    public setOrMerge(key: string, data: Object): void {
        ...
        if (this.exists(cleanKey)) {
            const existingData = this.metadata.get(cleanKey);
            data = { ...existingData, ...data };
        }

        this.metadata.set(cleanKey, data);
    }

    public remove(key: string): boolean {
        const cleanKey = Metadata.formatKey(key);
        return this.metadata.delete(cleanKey);
    }

    public get<T>(key: string): T {
        const cleanKey = Metadata.formatKey(key);
        return this.metadata.get(cleanKey) as T;
    }
    ...
    public exists(key: string): boolean {
        const cleanKey = Metadata.formatKey(key);
        return this.metadata.has(cleanKey);
    }
